...
async def update_telegram_city(user: str = Depends(check_token), city: str = Form(default=None), telegram_id: int = Form(default=None),
...
async def get_forecast(token = Depends(check_token), city_id: int = Form(...), forecast_range: str = Form(...), short_flag: bool = Form(default=False),
